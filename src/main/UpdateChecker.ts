...
export default class UpdateChecker {
  ...
  async check() {
    ...
    let outdated = gt(latestVersion, currentVersion);

    // Set appropriate prompt
    if (outdated && translated) {
  ...
  async showPrompt() {
    ...
    if (responseString === "Download") {
      if (this.alertDownloadUrl === null) {
        await shell.openExternal(GITHUB_BASE_URL + "/releases/latest");
      } else {
        await shell.openExternal(this.alertDownloadUrl);
      }
    } else if (responseString === "View Changelog") {
      await shell.openExternal(GITHUB_BASE_URL + "/releases");
    }
  }
}
