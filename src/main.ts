...
app.on("ready", async () => {
  createWindow();
  if (getPref("updates", "startup") !== "startup") return;
  await updateChecker.check();
  if (updateChecker.getShouldPrompt()) {
...
ipcMain.handle("getPref", async (event, pref: string, def: any) => {
  return getPref(pref, def);
});

ipcMain.handle("setPref", async (event, pref: string, value: any) => {
  return setPref(pref, value);
});
  ...
  },
  {
    ...
    submenu: [
      ...
      },
      {
        label: "GitHub",
        click: () => {
          shell.openExternal(GITHUB_BASE_URL);
        },
      },
      {
        label: "Documentation",
        click: () => {
          shell.openExternal(GITHUB_BASE_URL + "/wiki");
        },
      },
      {
